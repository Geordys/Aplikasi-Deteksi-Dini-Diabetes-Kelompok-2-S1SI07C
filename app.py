...
print(f"Jumlah baris (rows): {data.shape[0]}")
print(f"Jumlah kolom (columns): {data.shape[1]}\n")

print("=== NAMA ATRIBUT (KOLOM) ===")
print(data.columns.tolist(), "\n")

print("=== TIPE DATA SETIAP ATRIBUT ===")
print(data.dtypes, "\n")

print("=== INFO DETAIL DATAFRAME ===")
data.info()
print("\n")
...
# 2.4 Statistik deskriptif untuk variabel numerik
print("=== STATISTIK DESKRIPTIF VARIABEL NUMERIK ===")
desc_num = data[numerical_cols].describe().T  # T biar kebalik: baris = variabel
print(desc_num, "\n")

# Bisa tambahin median secara eksplisit kalau mau
median_vals = data[numerical_cols].median()
print("=== MEDIAN SETIAP VARIABEL NUMERIK ===")
print(median_vals, "\n")
...
# Berdasarkan pengecekan, jika ada missing values kita hapus atau imputasi
# 1. Handling Missing Value
if data.isnull().values.any():
    data = data.dropna() # Menghapus baris dengan nilai kosong (jika ada)
    print("✅ Poin 1: Missing Value telah dihapus.")
else:
    print("✅ Poin 1: Tidak ada missing value.")
...
print("✅ Poin 2: Data Cleaning (Encoding) selesai.")
...
print("✅ Poin 3: Outlier pada Age telah ditangani.")

# 4. Data Reduction
data = data.drop_duplicates()
print(f"✅ Poin 4: Data Reduction (Hapus Duplikat) selesai. Sisa baris: {len(data)}")
...
print("✅ Poin 5: Normalization (Scaling) selesai.")

# 6. Sampling (Data Splitting)
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.2, random_state=42, stratify=y)
print(f"✅ Poin 6: Sampling (80:20) selesai. Data Latih: {len(X_train)}, Data Uji: {len(X_test)}")


# 7. Encoding Data Kategorikal

# Kita gunakan LabelEncoder untuk mengubah 'Yes'/'No' atau 'Male'/'Female' menjadi 1/0
le = LabelEncoder()

# List kolom kategorikal (semua kecuali 'Age')
columns_to_encode = data.select_dtypes(include=['object']).columns

for col in columns_to_encode:
    data[col] = le.fit_transform(data[col])

print("Encoding selesai. Semua data kini bertipe numerik.")


# 8. Standarisasi Data (Scaling)

# Karena 'Age' memiliki rentang (20-90) sedangkan fitur lain (0-1),
# kita perlu standarisasi agar model tidak bias.
scaler = StandardScaler()

# Kita hanya menskalakan 'Age' atau bisa seluruh fitur independen (X)
# Di sini kita siapkan X (fitur) dan y (target)
X = data.drop('class', axis=1)
y = data['class']
...
#         margin-bottom: 20px;
#     ">
#         <span>🌐 <b>Status:</b> {"AI Model Ready" if en else "Sistem AI Siap"}</span>
...
#         </marquee>
#         <span>📅 {pd.Timestamp.now().strftime('%d %B %Y')}</span>
#     </div>
# """, unsafe_allow_html=True)
# 
...
#     # Menggabungkan Logo 🏥 dengan Judul dalam satu baris rapi
#     st.markdown(f"""
...
#         theme_label = ("☀️ Light" if en else "☀️ Siang") if st.session_state.theme == 'dark' else ("🌙 Dark" if en else "🌙 Malam")
# 
...
# st.markdown(f"**📍 {"Operational Unit" if en else "Unit Operasional"}**")
...
# st.markdown(f"**🎯 {"Model Reliability" if en else "Keandalan Model"}**")
# st.progress(auc_val)
...
# st.markdown(f"**💡 {"Daily Health Tip" if en else "Tips Kesehatan"}**")
# tips = [
...
#     v1, v2 = st.columns(2)
#     with v1:
#         st.markdown(f'<div class="white-card"><h4>{"🚻 Gender Composition" if en else "🚻 Komposisi Gender"}</h4>', unsafe_allow_html=True)
...
#     with v2:
#         st.markdown(f'<div class="white-card"><h4>{"📈 Age & Diagnosis Distribution" if en else "📈 Distribusi Usia & Diagnosa"}</h4>', unsafe_allow_html=True)
...
# # ===================================================
# 
#     st.markdown(f'<div class="white-card"><h4>{"🩺 AI Predictor (16 Symptoms)" if en else "🩺 AI Prediktor (16 Gejala)"}</h4>', unsafe_allow_html=True)
# 
...
#             st.warning(f"⚠️ **Penjelasan Medis (Medical Explanation)**: Risiko tinggi terdeteksi. Segera konsultasi ke dokter untuk pemeriksaan lebih lanjut.")
#         else:
#             st.success(f"✅ **Penjelasan Medis (Medical Explanation)**: Risiko rendah terdeteksi. Tetap jaga pola makan dan gaya hidup sehat.")
# 
#     st.markdown('</div>', unsafe_allow_html=True)
# 
# 
...
# else:
#     # --- HEATMAP (Sudah Benar) ---
#     st.markdown(f'<div class="white-card"><h4>{"📊 Correlation Heatmap" if en else "📊 Heatmap Korelasi"}</h4>', unsafe_allow_html=True)
...
#     with c1:
#         # --- FEATURE IMPORTANCE ---
#         st.markdown(f'<div class="white-card"><h4>{"🎯 Feature Importance" if en else "🎯 Kepentingan Fitur"}</h4>', unsafe_allow_html=True)
...
#     with c2:
#         # --- ROC CURVE ---
#         st.markdown(f'<div class="white-card"><h4>{"📈 ROC Curve" if en else "📈 Kurva ROC"}</h4>', unsafe_allow_html=True)
#         fpr, tpr, _ = roc_curve(y_test, y_prob)
